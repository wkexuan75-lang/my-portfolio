...
export type ImaHeatmapPhonePreviewProps = {
  className?: string;
  shellClassName?: string;
};

/** Standalone IMA “Pulse” floor map — no app contexts or providers. */
export function ImaHeatmapPhonePreview({
  className,
  shellClassName,
}: ImaHeatmapPhonePreviewProps) {
  const [activeFloor, setActiveFloor] =
    useState<(typeof FLOORS)[number]["id"]>("ground");
  ...
  return (
    ...
    >
      <motion.div
        className={[
          "relative flex min-h-0 w-full flex-1 flex-col overflow-hidden bg-[#050505] font-sans",
          shellClassName,
        ]
          .filter(Boolean)
          .join(" ")}
      ...
      >
        ...
        <div className="relative z-0 flex min-h-0 flex-1 justify-center px-4 pb-24">
          <svg
            viewBox="0 0 100 85"
            className="aspect-[100/85] w-full max-w-lg self-center"
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-4 z-20 flex justify-center px-6">
          <button
            type="button"
            className="pointer-events-auto relative rounded-full px-8 py-4 text-base font-medium text-white transition-transform active:scale-[0.98]"
            style={{
